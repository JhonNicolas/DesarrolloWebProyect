...
$con = fnConnect($msg);
$sql = "select a.COD_admin, a.nombre_admin from administrador a;";
...
$mensaje=null;
    if(isset($_POST["enviar"])){
        //capturando datos
        $reg["COD_admin"] = $_POST["COD_admin"];
        $reg["nombre_admin"] = $_POST["nombre_admin"];
        InsertarAdmin($reg, $mensaje, $error);
    }
    function InsertarAdmin($reg, &$mensaje, &$error){
        $con = fnConnect($msg);
        mysqli_query($con, "start transaction");
        $sqlinsert = "insert into administrador(COD_admin,nombre_admin)values ('{$reg["COD_admin"]}','"
        . "{$reg["nombre_admin"]});";
...
<html>
    ...
    <header>
       <body class="p-5 m-0 border-0 bd-example">                 
        ...
        <h1 align="left">TABLA DE ADMINISTRADORES</h1>
       ...
       <div class="container-fluid">
        ...
        <table class="table table-dark table-striped table-hover table_id" id="tblProductos">
            <thead>
                <tr class="Lineas">
                    <th colspan="7" class="colorCabecera"> LISTA DE ADMINISTRADORES </th>                   
                </tr>
                <tr>
